# 3D Gaussain Splatting Optimization Project
3D Gaussian Splatting (3DGS) has emerged as a powerful method for novel view synthesis, combining the photorealism of neural radiance fields with the efficiency of explicit pointbased representations. Despite its high rendering quality and speed, 3DGS suffers from a substantial memory footprint due to the need to store millions of Gaussian primitives with high-dimensional attributes. In this project, we propose two complementary strategies to address this challenge: octreebased compression and vector quantization. Octree compression reduces the number of primitives by merging nearby points in the initial point cloud, while vector quantization clusters continuous attributes into compact codebooks, drastically reducing storage without sacrificing quality. Applied to a high-resolution scene from the Tanks and Temples dataset, our method achieves up to a 4.32× reduction in memory, with a 1.21× increase in training speed, and maintains competitive visual fidelity. These results demonstrate that our compression techniques enable scalable and efficient deployment of 3DGS, making it more viable for resource-constrained applications such as mobile AR and real-time rendering. 

[Project Report Link](https://drive.google.com/file/d/1EIoIX0ACcvGAhBOPgAburSQGFJVSknzd/view?usp=sharing)
